...
foreach($biglist as $word) 
{      
    $biglist[$i]=$morphy->lemmatize("$word",phpMorphy::IGNORE_PREDICT);
    $i++;
}
} catch(phpMorphy_Exception $e) {
 ...
 //удаляем из массива значения = false
 //которые были получены в результате того, что
 //php-morphy не распознал некоторые слова
for ($i=0; $i< count($biglist);$i++) 
    {
        if ($biglist[$i] !== false)
        {
            $biglisttmp[$i]=$biglist[$i];
        }
    }   
unset($biglist);
 $i=0;  
foreach ($biglisttmp as $element)
{
    $biglist[$i] = $element;
    $i++;
}
...
for($j=0;$j<count($massiv_chastota_sort);$j++)
 {
     echo $massiv_chastota_sort[$j][0];
     echo ("<br />");
     echo $massiv_chastota_sort[$j][2];
     echo ("<hr />");
     for ($i=0;$i<count($massiv_chastota_sort[$j][1]);$i++)
     {
         $massiv_chastota_sort[$j][1][$i] = iconv("CP1251", "UTF-8",$massiv_chastota_sort[$j][1][$i]);
         echo $massiv_chastota_sort[$j][1][$i];
         echo ("<br />");
     }
 }*/
...
// ф-ция нахождения стоп-слов в массиве            
 function stop_words_delete(&$words_array_element)
 {
     ...
     ("А",
     "БЫ","БЕЗ","БОЛЕЕ","БЫТЬ","Б",
     "В","ВАШ","ВО","ВОТ","ВСЕ","ВПОЛНЕ","ВПРОЧЕМ","ВРОДЕ","ВЫ","ВЕСЬ",
     "ГДЕ",
     "ДЛЯ","ДОВОЛЬНО", "ДА","ДО",
     "ЕЕ","ЕГО","ЕСЛИ","ЕЩЕ",
     "ЖЕ",
...
//подключаем алгоритм разбиения текста на предложения
